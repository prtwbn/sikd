                        ...
                        <div class="form-group">
                          ...
                          <select class="form-control @error('id_prodi') is-invalid @enderror" id="exampleFormControlSelect1" name="id_prodi">
                            @foreach ($prodi as $itemP)
                              <option value="{{ $itemP->id }}" {{ $item->id_prodi == $itemP->id ? 'selected' : '' }}>
                                {{ $itemP->prodi }}
                              </option>
                            @endforeach
                          </select>
                      ...
                      <div class="col-md-3">
                        <div class="form-group">
                          ...
                          <select class="form-control @error('id_dosen') is-invalid @enderror" id="exampleFormControlSelect1" name="id_dosen">
                            ...
                            <option value="{{ $item->id_dosen }}">{{ $item->dosen ? $item->dosen->nama : '-' }}</option>
                            <!-- Menampilkan daftar dosen lain untuk dipilih -->
                            @foreach ($dosenWali as $itemD)
                              <option value="{{ $itemD->id }}" {{ $item->id_dosen == $itemD->id ? 'selected' : '' }}>
                                {{ $itemD->nama }}
                              </option>
                            @endforeach
                          </select>
